...
def index():
    logger.debug("""Case-Data Web Pages""")
    ...
    try:
        ...
        if x.get('item') is not None:
            ...
            return render_template('viewone/casedata/' + x.get(
                'item') + '.html', data=x.get('data'))

    except Exception as e:
        data['error'] = "Error Index-Template Case-Data {}".format(e)
...
def data(filename):
    ...
    try:
        ...
        metadata = locate('models.casedata.' + filename)
        ...
        data['error'] = "Error Index-Data Case-Data-Core {}".format(e)
        logger.critical(data['error'])
...
def action(filename):
    ...
    try:
        ...
        data['error'] = "Error Index-Action Case-Data-Core {}".format(e)
        logger.critical(data['error'])
...
def stats(filename):
    ...
    try:
        ...
        data['error'] = "Error Index-Stats Case-Data-Core {}".format(e)
        logger.critical(data['error'])
